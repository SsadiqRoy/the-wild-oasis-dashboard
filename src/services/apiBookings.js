...
export async function getBookings({ filter, sortBy, page }) {
  let query = supabase
    .from('Bookings')
    .select('id,created_at,startDate,endDate,numNights,numGuests,totalPrice,status,Cabins(name), Guests(fullName,email)', { count: 'exact' });
...
export async function deleteBooking(id) {
  ...
  if (error) {
    console.error('deleting booking', error);
