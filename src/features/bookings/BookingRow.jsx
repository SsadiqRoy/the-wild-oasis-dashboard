import styled from 'styled-components';
import { format, isToday } from 'date-fns';

import Tag from '../../ui/Tag';
import Table from '../../ui/Table';

import { formatCurrency } from '../../utils/helpers';
import { formatDistanceFromNow } from '../../utils/helpers';
import Menus from '../../ui/Menus';
import { HiArrowLeftOnRectangle, HiArrowRightOnRectangle, HiEye, HiTrash } from 'react-icons/hi2';
import { useNavigate } from 'react-router-dom';
import Modal from '../../ui/Modal';
import { useCheckOutBooking, useDeleteBooking } from './bookingHooks';
import ConfirmDelete from '../../ui/ConfirmDelete';

const Cabin = styled.div`
  font-size: 1.6rem;
  font-weight: 600;
  color: var(--color-grey-600);
  font-family: 'Sono';
`;

const Stacked = styled.div`
  display: flex;
  flex-direction: column;
  gap: 0.2rem;

  & span:first-child {
    font-weight: 500;
  }

  & span:last-child {
    color: var(--color-grey-500);
    font-size: 1.2rem;
  }
`;

const Amount = styled.div`
  font-family: 'Sono';
  font-weight: 500;
`;

function BookingRow({ booking }) {
  const navigate = useNavigate();
  const { checkout } = useCheckOutBooking();
  const { deleteBooking, isDeletingBooking } = useDeleteBooking();

  const {
    id: bookingId,
    // created_at,
    startDate,
    endDate,
    numNights,
    // numGuests,
    totalPrice,
    status,
    Guests: { fullName: guestName, email },
    Cabins: { name: cabinName },
  } = booking;

  // console.log('i have rendered', bookingId);

  const statusToTagName = {
    unconfirmed: 'blue',
    'checked-in': 'green',
    'checked-out': 'silver',
  };
  const buttonStyle = {
    width: ' 100%',
    textAlign: ' left',
    background: ' none',
    border: ' none',
    padding: ' 1.2rem 2.4rem',
    fontSize: ' 1.4rem',
    transition: ' all 0.2s',

    display: ' flex',
    alignItems: ' center',
    gap: ' 1.6rem',
  };

  const handlerCheckOut = () => checkout({ id: bookingId, data: { status: 'checked-out' } });
  const handleDelete = () => deleteBooking(bookingId);

  return (
    <Table.Row>
      <Cabin>{cabinName}</Cabin>

      <Stacked>
        <span>{guestName}</span>
        <span>{email}</span>
      </Stacked>

      <Stacked>
        <span>
          {isToday(new Date(startDate)) ? 'Today' : formatDistanceFromNow(startDate)} &rarr; {numNights} night stay
        </span>
        <span>
          {format(new Date(startDate), 'MMM dd yyyy')} &mdash; {format(new Date(endDate), 'MMM dd yyyy')}
        </span>
      </Stacked>

      <Tag type={statusToTagName[status]}>{status.replace('-', ' ')}</Tag>

      <Amount>{formatCurrency(totalPrice)}</Amount>

      <div>
        <Modal>
          <Menus.Menu>
            <Menus.Toggle id={bookingId} />

            <Menus.List id={bookingId}>
              <Menus.Button icon={<HiEye />} onclick={() => navigate(`/bookings/${bookingId}`)}>
                Details
              </Menus.Button>

              {status === 'unconfirmed' && (
                <Menus.Button icon={<HiArrowRightOnRectangle />} onclick={() => navigate(`/checkin/${bookingId}`)}>
                  Check In
                </Menus.Button>
              )}

              {status === 'checked-in' && (
                <Menus.Button icon={<HiArrowLeftOnRectangle />} onclick={handlerCheckOut}>
                  Check Out
                </Menus.Button>
              )}

              <Modal.Open opens={bookingId}>
                <button style={buttonStyle}>
                  <HiTrash /> Delete
                </button>
              </Modal.Open>
            </Menus.List>
          </Menus.Menu>

          <Modal.Window name={bookingId}>
            <ConfirmDelete resourceName="booking" disabled={isDeletingBooking} onConfirm={handleDelete} />
          </Modal.Window>
        </Modal>
      </div>
    </Table.Row>
  );
}

export default BookingRow;
